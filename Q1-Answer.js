...
function searchCafes(name) {
  let matchCafes = [];
      let matchPlace = [];
      for(let i=0; i<cafes.length; i++) {
          if(cafes[i].name.includes(name)){
              matchCafes = [...matchCafes, cafes[i]];
          }
      }
      for(let i=0; i<matchCafes.length; i++) {
          for(let j=0; j<places.length; j++) {
              if(matchCafes[i].place_id === places[j].id) {
              
                  matchPlace.push({
                      "name": matchCafes[i].name,
  ...
  console.log(searchCafes('Avenue'));